...
/**
 * Helper functions for course_overview_past block
 *
...
define('BLOCKS_COURSE_OVERVIEW_PAST_SHOWCATEGORIES_NONE', '0');
define('BLOCKS_COURSE_OVERVIEW_PAST_SHOWCATEGORIES_ONLY_PARENT_NAME', '1');
define('BLOCKS_COURSE_OVERVIEW_PAST_SHOWCATEGORIES_FULL_PATH', '2');

/**
 * Sets user preference for maximum courses to be displayed in course_overview_past block
 *
 * @param int $number maximum courses which should be visible
 */
function block_course_overview_past_update_mynumber($number) {
    set_user_preference('course_overview_past_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in course_overview_past block
 *
 * @param array $sortorder list of course ids
 */
function block_course_overview_past_update_myorder($sortorder) {
    ...
    set_user_preference('course_overview_past_course_sortorder', $value);
}

/**
 * Gets user course sorting preference in course_overview_past block
 *
 * @return array list of course ids
 */
function block_course_overview_past_get_myorder() {
    if ($value = get_user_preferences('course_overview_past_course_sortorder')) {
        return explode(',', $value);
    }
    // If preference was not found, look in the old location and convert if found.
    $order = array();
    if ($value = get_user_preferences('course_overview_past_course_order')) {
        $order = unserialize($value);
        block_course_overview_past_update_myorder($order);
        unset_user_preference('course_overview_past_course_order');
    }
    return $order;
}
 ...
 */
function block_course_overview_past_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_course_overview_past', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_course_overview_past', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_course_overview_past', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in course_overview_past block
 *
 * @param bool $showallcourses if set true all courses will be visible.
 * @return int maximum number of courses
 */
function block_course_overview_past_get_max_user_courses($showallcourses = false) {
    ...
    if (empty($config->forcedefaultmaxcourses)) {
        if ($showallcourses) {
            ...
            $limit = get_user_preferences('course_overview_past_number_of_courses', $limit);
        }
    }
    return $limit;
}
 ...
 */
function block_course_overview_past_get_sorted_courses($showallcourses = false) {
    global $USER;

    $limit = block_course_overview_past_get_max_user_courses($showallcourses);

    //$courses = enrol_get_my_courses();
	$courses = enrol_get_my_courses_past();
    $site = get_site();
    ...
    $order = block_course_overview_past_get_myorder();
...
function enrol_get_my_courses_past($fields = NULL, $sort = 'visible DESC,sortorder ASC', $limit = 0) {
    global $DB, $USER;
