...
/**
 * course_overview_past block rendrer
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_course_overview_past_renderer extends plugin_renderer_base {

    /**
     * Construct contents of course_overview_past block
     *
     * @param array $courses list of courses in sorted order
     * @param array $overviews list of course overviews
     * @return string html to be displayed in course_overview_past block
     */
    public function course_overview_past($courses, $overviews) {
        ...
        if ($this->page->user_is_editing() && (count($courses) > 1)) {
            $userediting = true;
            $this->page->requires->js_init_call('M.block_course_overview_past.add_handles');
        ...
        if ($ismovingcourse) {
            ...
            $html .= get_string('movingcourse', 'block_course_overview_past', $a);
            $html .= $this->output->box_end();

            $moveurl = new moodle_url('/blocks/course_overview_past/move.php',
            ...
            $movetofirsticon = html_writer::empty_tag('img',
                    array('src' => $this->output->pix_url('movehere'),
                        'alt' => get_string('movetofirst', 'block_course_overview_past', $courses[$movingcourseid]->fullname),
        ...
        foreach ($courses as $key => $course) {
            ...
            $html .= $this->output->box_start('courseboxp', "course-{$course->id}");
            $html .= html_writer::start_tag('div', array('class' => 'course_title_past'));
            // If user is editing, then add move icons.
            if ($userediting && !$ismovingcourse) {
                $moveicon = html_writer::empty_tag('img',
                        array('src' => $this->pix_url('t/move')->out(false),
                            'alt' => get_string('movecourse', 'block_course_overview_past', $course->fullname),
            ...
            if (!empty($config->showchildren) && ($course->id > 0)) {
                // List children here.
                if ($children = block_course_overview_past_get_child_shortnames($course->id)) {
            ...
            if ($config->showcategories != BLOCKS_COURSE_OVERVIEW_PAST_SHOWCATEGORIES_NONE) {
                ...
                if ($currentcategory !== null) {
                    ...
                    if ($config->showcategories == BLOCKS_COURSE_OVERVIEW_PAST_SHOWCATEGORIES_FULL_PATH) {
            ...
            if ($ismovingcourse) {
                ...
                $movehereicon = html_writer::empty_tag('img',
                        array('src' => $this->output->pix_url('movehere'),
                            'alt' => get_string('moveafterhere', 'block_course_overview_past', $a),
                            'title' => get_string('movehere')));
        ...
        return html_writer::tag('div', $html, array('class' => 'course_listp'));
    }
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_course_overview_past', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_course_overview_past'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
        $url = new moodle_url('/my/index.php');
        $select = new single_select($url, 'mynumberp', $options, block_course_overview_past_get_max_user_courses(), array());
        $select->set_label(get_string('numtodisplay', 'block_course_overview_past'));
    ...
    public function hidden_courses($total) {
        ...
        if (!empty($config->forcedefaultmaxcourses)) {
            $output .= get_string('hiddencoursecount'.$plural, 'block_course_overview_past', $total);
        } else {
            $a = new stdClass();
            $a->coursecount = $total;
            $a->showalllink = html_writer::link(new moodle_url('/my/index.php', array('mynumberp' => block_course_overview_past::SHOW_ALL_COURSES_P)),
                    get_string('showallcourses'));
            $output .= get_string('hiddencoursecountwithshowall'.$plural, 'block_course_overview_past', $a);
        }
    ...
    protected function collapsible_region_start($classes, $id, $caption, $userpref = '', $default = false) {
        ...
        $this->page->requires->js_init_call('M.block_course_overview_past.collapsible', array($id, $userpref, get_string('clicktohideshow')));
    ...
    public function welcome_area($msgcount) {
        ...
        $output .= $this->output->heading(get_string('welcome', 'block_course_overview_past', $USER->firstname));

        if (!empty($CFG->messaging)) {
            $plural = 's';
            if ($msgcount > 0) {
                $output .= get_string('youhavemessages', 'block_course_overview_past', $msgcount);
                if ($msgcount == 1) {
                    $plural = '';
                }
            } else {
                $output .= get_string('youhavenomessages', 'block_course_overview_past');
            }
            $output .= html_writer::link(new moodle_url('/message/index.php'),
                    get_string('message'.$plural, 'block_course_overview_past'));
        }
        $output .= $this->output->box_end();
